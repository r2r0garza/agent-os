...
def execute_model_harness(
    ...
    *,
    ...
    """Execute one no-tool model turn for a task run through a pinned model profile.
    ...
    then invokes the pinned OpenAI-compatible endpoint and persists redacted
    lifecycle evidence into the existing audit/observability structures.
    """
    ...
    if response is None or not response.is_success:
        ...
        session.add(
            AuditEvent(
                ...
                payload={"thread_id": thread_id, "attempts": attempts, "diagnostic": diagnostic},
            )
        )
        session.flush()
        record_observability(
            ...
            status="failed",
            attributes={"thread_id": thread_id, "attempts": attempts, "diagnostic": diagnostic},
        )
    ...
    try:
        ...
        session.add(
            AuditEvent(
                ...
                run_id=run.id,
                event_type="harness.invocation_failed",
                payload={"thread_id": thread_id, "attempts": attempts, "diagnostic": "malformed_response"},
            )
        )
        session.flush()
        record_observability(
            ...
            operation_name="harness.invocation_failed",
            status="failed",
            attributes={"thread_id": thread_id, "attempts": attempts, "diagnostic": "malformed_response"},
        )
        raise HarnessExecutionError(
